...
class Recipe extends CI_Controller
{
    ...
    public function add()
    {
        ...
        $data['all_units'] = $this->units->get_units_list();
        //echo '<pre>'; print_r($data['all_units']); exit;
        $head['title'] = "Add Recipe";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('recipe/recipe-add', $data);
        $this->load->view('fixed/footer');
    }
...
&nbsp;<a href="' . base_url() . 'products/edit?id=' . $pid . '"  class="btn btn-purple btn-sm"><span class="fa fa-edit"></span>' . $this->lang->line('Edit') . '</a><div class="dropdown-divider"></div>&nbsp;<a href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm  delete-object"><span class="fa fa-trash"></span>' . $this->lang->line('Delete') . '</a>
    ...
    {
        $product_name = $this->input->post('recipe_name', true);
        $catid = $this->input->post('product_cat');
        $warehouse = $this->input->post('product_warehouse');
        $product_code = $this->input->post('recipe_code');
        $product_price = numberClean(array_sum($this->input->post('total_item_price')));
        $product_wholesale_price = 0; //numberClean($this->input->post('product_wholesale_price'));
        $factoryprice = 0; //numberClean($this->input->post('fproduct_price'));
        $taxrate = 0; //numberClean($this->input->post('product_tax', true));
        $disrate = 0; //numberClean($this->input->post('product_disc', true));
        $product_qty = 100000; //numberClean($this->input->post('product_qty', true));
        $product_qty_alert = 5; numberClean($this->input->post('product_qty_alert'));
        ...
        $type = '0'; //$this->input->post('product_serial');

        //echo '<pre>'; print_r($_POST);exit;
        $product_name_array = $_POST['product_name'];
        for($i=0; $i < count($product_name_array); $i++){            
            $data[$i] = array(
                'ingredient_id' => $_POST['pid'][$i],
                'ingredient_name' => $_POST['product_name'][$i],
                'ingredient_qty' => $_POST['product_qty'][$i],
                'ingredient_per_price' => $_POST['product_price'][$i],
                'ingredient_price' => $_POST['total_item_price'][$i],
                'unit' => $_POST['unit'][$i],
                'loc' => $this->aauth->get_user()->loc
            );
        }        
        if ($catid) {
            $this->recipe->addnew($catid, $warehouse, $product_name, $product_code, $product_price, $product_wholesale_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $v_type, $v_stock, $v_alert, $wdate, $code_type, $w_type, $w_stock, $w_alert, $sub_cat, $brand, $serial, $data);
        } 
    }
