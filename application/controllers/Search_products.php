    ...
    public function v2_pos_search()
    {
        ...
        $flag_p=true;
            $bar = " (geopos_products.barcode = '" . (substr($name, 0, -1)) . "' OR geopos_products.barcode LIKE '" . $name . "%')";
               ...
               $query = "SELECT geopos_products.*  FROM geopos_products $join WHERE " . $qw . "$bar AND (geopos_products.qty>0) ORDER BY geopos_products.product_name AND (geopos_products.product_type = 0)  LIMIT 6";
               $p_class='v2_select_pos_item_bar';

        } elseif ($enable_bar=='false' OR !$enable_bar ) {
            $like = $bar ? "(UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%' $bar 
            OR geopos_products.product_code LIKE '" . strtoupper($name) . "%') AND" : "" ;

            
            $flag_p=true;
            if ($billing_settings['key1'] == 2) {

                $query = "SELECT geopos_products.*,geopos_product_serials.serial FROM geopos_product_serials  LEFT JOIN geopos_products  ON geopos_products.pid=geopos_product_serials.product_id $join WHERE " . $qw ." ".$like."
                 geopos_product_serials.serial LIKE '" . strtoupper($name) . "%'  AND (geopos_products.qty>0) AND (geopos_products.product_type = 0) LIMIT 18";

            } else {

                $query = "SELECT geopos_products.* $e FROM geopos_products $join WHERE " . $qw ." ". $like."
                 (geopos_products.qty>0) AND (geopos_products.product_type = 0) ORDER BY geopos_products.product_name LIMIT 18";
            }
